...
const CHAT_ONLY = ["glm-5.3", "glm-5.2", "kimi-k3", "kimi-k2.7-code", "kimi-k2.6", "mimo-v2.5", "mimo-v2.5-pro"];
...
describe("OpenCode Go model catalog", () => {
  it("matches the documented model IDs", () => {
    ...
    expect(ids).toEqual([
      "glm-5.2", "glm-5.3", "kimi-k3", "kimi-k2.7-code", "kimi-k2.6",
...
describe("OpenCode Go reasoning contracts", () => {
  it("exposes only low/high/max for GLM-5.3 and Kimi K3", () => {
    expect(getThinkingLevels("opencode-go", "glm-5.3")).toEqual(["low", "high", "max"]);
    expect(getThinkingLevels("opencode-go", "kimi-k3")).toEqual(["low", "high", "max"]);
  });

  it("sends reasoning_effort without a disable-thinking field", () => {
    for (const model of ["glm-5.3", "kimi-k3"]) {
      const body = { reasoning_effort: "max" };
      applyThinking("openai", model, body, "opencode-go");
      expect(body).toEqual({ reasoning_effort: "max" });
    }
  });

  it("maps generic client effort names to the provider's enum", () => {
    const body = { reasoning_effort: "xhigh" };
    applyThinking("openai", "glm-5.3", body, "opencode-go");
